const { InvalidRequest, InvalidClientAuth } = require('../../util/error');
      ...
      async function auth(ctx, next) {
        ...
        switch (clientMethod) { // eslint-disable-line default-case
          ...
          case 'client_secret_jwt':
            ...
            await tokenJwtAuth(
              ctx, ctx.oidc.client.symmetricKeyStore,
          ...
          case 'private_key_jwt':
            await tokenJwtAuth(
              ctx, ctx.oidc.client.asymmetricKeyStore,
              signingAlg ? [signingAlg] : instance(provider).configuration(`${endpoint}EndpointAuthSigningAlgValues`).filter((alg) => !alg.startsWith('HS')),
            );
          ...
          case 'self_signed_tls_client_auth': {
            ...
            const expected = thumbprint(cert);
