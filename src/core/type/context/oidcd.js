...
const { getResponseMode } = require("../response/mode");

const COOKIES = Symbol();

class OIDCContext {
  static requestParamClaims = null;
  static accessToken = null;
  static fapiProfile = null;
  ...
  get cookies() {
    if (!this[COOKIES]) {
      ...
      this[COOKIES].secure =
        !this[COOKIES].secure && this.ctx.secure ? true : this[COOKIES].secure;
    }
  ...
  urlFor(name, opt) {
    const mountPath =
      (this.ctx.req.originalUrl &&
        this.ctx.req.originalUrl.substring(
          0,
  ...
  get requestParamScopes() {
    return new Set(
      this.params.scope ? this.params.scope.split(" ") : undefined
    );
  }
  ...
  get responseMode() {
    ...
    if (this.params.response_type !== undefined) {
      return getResponseMode(this.params.response_type);
    }
  ...
  getAccessToken({
    acceptDPoP = false,
    acceptQueryParam = !fapi.enabled && acceptQueryParamAccessTokens,
  } = {}) {
    if (this.#accessToken) {
    ...
    const mechanisms = omitBy(
      {
        body:
          ...
          get(ctx.oidc, "body.access_token"),
