...
const os = require("os");
const {escape} = require("lodash/_escapeHtmlChar")

const debug = require("../../util/debug");
const Store = require("../../store/index");

const { nanoid } = require("../../util/nanoid");
const { defaultInteractionPolicy } = require("../interaction/policy");
const errors = require("../../util/error");
const KeyStore = require("../../util/key_store");
const { fromBuffer } = require("../../util/encode_base64_url");
...
const ks = new KeyStore([
  {
    ...
    kid: "kid-for-test",
    kty: "RSA",
    n: "7kYPcwJ9YYMNNedgLB5H-x7iryueM8SIbjbjcBcZHrmXvE4r65AHej8jr0XJGwKbrbbueSS_fqIZnnj1Es3tl5_SPFjVOR3b1JXpje8BMdymQQFuNV8rDG05BUM0RIQqq01pZ7dEC6TdlQKXWLFmfi-sdr_07ENb2mzDzNoHRRcHHVkXoM_YygTqDQKzjj77nHoQH8638rW2i67cDk6mAgBupJrctE9Iad_ujeZ5_Ogoy0UDNnkLGX7fBHPYbbyABDN2Fops88Iq_GtHIEMCfkC3gL7NlwH0NO06IIADQ8fdP3CggJmH-vrwVw0HzDMGqjR4HlLAgpphQTEwb6FnEw",
    e: "AQAB",
    d: "QkWSPnbO0LRWlEuKYDr74kiw2-_RzKw5Fg6GK8MA5Z7RKTrE1Hv22HV_Q1yxlqWfDy32HhOuf1YusfkKkPRdxPJa0xYSGBTJ89LOZv-24VtxklsHgCi8HHS0i1Ypz3TDOWoKiyJZ9rMgPjW-Spjs3OP9n_m78Yq6nYHq46swonI_0FmcnqAk8JVZ4rYFZ874aWeD4K9qH0n2RvGBCRTuRnUZYclC1TvWj7Kduv4aolzCwMB1-dWznV28KIQ1RCI8aSyAUHE-ssP74ChuE4mN-S3UbN90Uxc5AlmQfI5_pCsG17JhI9OEN9A-HzCGnCszdZ8haeilESzMP0E0UldJAQ",
    p: "-KT1bTcDOPegBpmH2IV2lAFjFM_tr5YHMjIqDFfXZ2MLOmv5It21cjjThhyeukJRpZJt4IhTWsS_ZGVOf4hVbIItjsOnBOoTKFwuOcUUT7WoMqDB9452uYVklheywFf19dEf0S_vq9afFpKjY5b6Pxw6jiXFlo5mIyMxDDmynqE",
    q: "9VKP3RT1-hasbHd4RRJFgiwj_y95g77g4tPDgWvkEbALZXK89pZL6_BQz9ejPSqOzPYrQDKBZ-58foW_pCs874GsVgk3y_okGlSs-2-i2hG9zFnIH_uvhLSNtosI-aDl6sNoOVjMzTVRX97Vwep7BI01pNu0G_Cx5C00R6o5rTM",
    dp: "qv1eHi4_1BROg1tf1WHq6tuNVMmtLhvhdP6UdgqOgnXVSebo3LZVSmiT5l2WNobW-SMa1tdJY1l8OUGF2yRZgpZ_V_dqy3FhnVbPH0FNt4c4VSDCY0kgknKOer1vWuDir5qR49ywWJ7upqpPHHS-QVpbVlfZarEgwjc4Sqz_DiE",
    dq: "z2c4x7HD7-ahaaZroQG8kDwfRsxtUGvdFwfvvwhnLmkGiYNCrLsOfu-SctWdZcYIKtJoaIMKksK2sAUFq5b-koKwH6D1clcNlNFH2d9c8D6VlyHEpv2TO3-mW9EaCmn0V9FS2UDxDQDpxkLPt300-fkkhDdXA1QwIXfyPjB1SK8",
    qi: "XKVRR4gmj78qA_SsTVba_wbOoYbn9OeWoDGWU__f91WVkVj85fhb-KfOL3yHccg4zBgfWVru5bZU1_0R46rds0HUWZFS0Kl81vdHBtJSdvcdNcZrox0Mem2ap6Jex7zrXD3Itq481FEn0JYwj02q_LKgtXnbXRkiooun4tAKz-s",
  },
]);
var store = null;
...
function defaultConfigShouldChange(name, msg) {
  if (!warned.has(name)) {
    warned.add(name);
    debug.info(
...
function defaultConfigMustChange(name, msg) {
  if (!warned.has(name)) {
    warned.add(name);
    debug.warn(
...
function defaultKeyStore() {
  defaultConfigMustChange("keyStore", "use the correct keys");
  return ks;
}

function defaultStore(model, storage) {
  if (store === null && storage) {
...
function IdTokenTTL(ctx, token, client) {
  defaultConfigShouldChange(
    "ttl.IdToken",
    "define the expiration for IdToken artifacts"
  );
  return 60 * 60; // 1 hour in seconds
}
...
function SessionTTL(ctx, session) {
  defaultConfigShouldChange(
    "ttl.Session",
    "define the expiration for Session artifacts"
  );
  return 14 * 24 * 60 * 60; // 14 days in seconds
}
...
function GrantTTL(ctx, grant, client) {
  defaultConfigShouldChange(
    "ttl.Grant",
    "define the expiration for Grant artifacts"
  );
  return 14 * 24 * 60 * 60; // 14 days in seconds
}
...
async function renderError(ctx, out, error) {
  defaultConfigShouldChange(
    "renderError",
    "customize the look of the error page"
  );
  ctx.type = "html";
  ctx.body = `<!DOCTYPE html>
    <head>
    ...
    <body>
      <div class="container">
        ...
        ${Object.entries(out)
          .map(
            ([key, value]) =>
              `<pre><strong>${key}</strong>: ${escape(value)}</pre>`
          )
          .join("")}
      </div>
    </body>
...
async function findAccount(ctx, sub, token) {
  ...
  defaultConfigMustChange("findAccount", "use your own account model");
  return {
    accountId: sub,
...
function getDefaults() {
  const defaults = {
    ...
    features: {
       ...
       * features.ciba
       *
       * title: [OpenID Connect Client Initiated Backchannel Authentication Flow - Core 1.0 - draft-03](https://openid.net/specs/openid-client-initiated-backchannel-authentication-core-1_0-03.html)
       *
      ...
      resourceIndicators: {
         ...
         *
         * example: Resource Server (API) with two scopes, an expected audience value, an Access Token TTL
       ...
       *
       * title: [Core 1.0](https://openid.net/specs/openid-connect-core-1_0.html#RequestObject) and [JWT Secured Authorization Request (JAR)](https://tools.ietf.org/html/draft-ietf-oauth-jwsreq-33) - Request Object
       *
